...
| `src/components/Header.astro` | The nav is now **eight** links; the design spec asked for four, with Gods and Hybrids behind a "Characters" entry. **The 375px pass settled the usability half and left the architecture half open.** Eight links wrap to two rows, which measured fine — no overflow, targets now 44px, header 26% of a 375×812 screen. Fitting one row is arithmetically impossible: the labels total 427px against 336px of usable width. So the remaining question is not layout, it is whether the site should have eight top-level sections — a content decision needing a new `/characters/` page, not a CSS fix. |
...
- [x] **Narrow-viewport pass at 375px.** Run 2026-09-06. Layout passed everywhere — no horizontal overflow on any page, `/worlds/` stacks, the chapter footer nav wraps as its 34rem query intends, and chapter prose reads well. What failed was **touch target size**: eleven controls under 44px (eight nav links at 19–20px, the language toggle at 33px, two footer links at 18px, the codex gate at 38px). Fixed by the `--tap` token and `@media (pointer: coarse)` blocks in `Header`, `Footer` and `SpoilerGate`. Header at 375×812 went 189px → 211px.
- [x] **Keyboard-only pass.** The skip link is first in tab order; tab order through the header is DOM order and correct; `<summary>` is natively focusable and opens on activation, with the gate body then visible. The lightbox is a native `<dialog>`, so Escape and the focus trap come from the platform.
- [x] `prefers-reduced-motion: reduce` — confirmed nothing animates. The site's only animation is `.reveal`, already opt-in behind `no-preference`; `Seam.astro` has none and documents that; the Two Suns backdrop is static.
